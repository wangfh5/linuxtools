...
| 命令 | 功能 |
|------|------|
| `asmgr add [source] [-a <agents>] [-s] [-g\|-p <dir>] [-c]` | 添加 skill 到中央目录并链接；省略 `source` 进入选择器；`-s` 把已有 subagent 链接到 `.claude/agents` |
...
不传 `source` 时，`asmgr add` 会从中央目录 `~/agent-settings/skills/` 进入交互式选择器，逐步选择 skills、agents、scope、link/copy。若本机有 `fzf` 且在 TTY 中运行，会使用多选界面；否则自动退化为纯 Bash 编号输入。

```bash
asmgr add                              # 逐步选择 skills / agents / scope / link-copy
asmgr add -a claude-code opencode -g   # 只选择 skills，安装到指定全局 agents
asmgr add -a codex -p ~/projects/foo -c # 只选择 skills，复制到指定项目 codex
```
...
| 格式 | 示例 | 说明 |
|------|------|------|
| GitHub URL | `https://github.com/coleam00/excalidraw-diagram-skill`、`https://github.com/anthropics/skills/tree/main/skills/skill-creator` | 从 GitHub 下载；支持 repo 根目录 skill 或 monorepo 子目录 skill |
